...
const NHIMATool: React.FC = () => {
  ...
  const createEmptyEmployee = (): Employee => ({
    ...
    monthPeriod: new Date().toISOString().slice(0, 7), // YYYY-MM format
    memberId: '',
    nrcPassport: '',
    surname: '',
    firstName: '',
  ...
  const updateEmployee = (index: number, field: keyof Employee, value: string) => {
    ...
    // Calculate NHIMA contributions when basic pay changes
    if (field === 'basicPay') {
      const basicPay = parseFloat(value) || 0;
      const employerShare = (basicPay * 0.01).toFixed(2); // 1% employer share
      const employeeShare = (basicPay * 0.01).toFixed(2); // 1% employee share
      updatedEmployees[index].employerShare = employerShare;
      updatedEmployees[index].employeeShare = employeeShare;
    }
    ...
    <div className="pt-16 min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-2xl p-8 shadow-lg mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">NHIMA Employee XLSX Generator</h1>
          <p className="text-lg text-gray-600 mb-6">
            Create NHIMA employee data files quickly and accurately. Add employee details and download as XLSX format.
          </p>
        ...
        <div className="space-y-6">
          {employees.map((employee, index) => (
            <div key={index} className="bg-white rounded-2xl p-6 shadow-lg relative">
              {/* Row Number Badge */}
              <div className="absolute -top-3 left-6 bg-green-600 text-white px-3 py-1 rounded-full text-sm font-semibold">
                Employee {index + 1}
              </div>

              {/* Form Grid */}
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
                {nhimaFields.map((field) => (
        ...
        <div className="bg-green-50 rounded-2xl p-6 mt-8">
          ...
          <ul className="text-green-800 space-y-2">
            ...
            <li>• NHIMA contributions are calculated at 1% each for employer and employee shares</li>
            <li>• Use the month picker to select the contribution period</li>
